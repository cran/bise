...
bise <- function(ndvi) {
    ...
    ndvi <- ifelse(ndvi < 0 | ndvi >= 1 | is.na(ndvi) == TRUE, 0, ndvi)
    ll <- 366
   ...
   if(check[30] < 0.1) { obsday <- NA }
   else { obsday <- 0 }
   
   if(is.na(obsday) == FALSE) { 
      ...
      ndvi.bb <- (max(res) + 0.11) * 0.5 
      dif.ndvi <- ifelse(res[1:(order(res, decreasing=TRUE)[1])], abs(res[1:(order(res, decreasing=TRUE)[1])] - ndvi.bb), NA)
      obsday <- order(dif.ndvi)[1]
   }
   return(obsday)
}
